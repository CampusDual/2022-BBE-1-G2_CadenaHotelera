...
class RoomTypeServiceTest {
	...
	void InsertBadColumnNameTest() {
		...
		assertEquals(MsgLabels.BAD_DATA, service.roomTypeInsert(data).getMessage());
	}
	...
	void InsertTestEmptyRoomName() {
		...
		assertEquals(MsgLabels.ROOM_TYPE_NAME_BLANK, service.roomTypeInsert(data).getMessage());
	}
	...
	void InsertTestDuplicate() {
		...
		assertEquals(MsgLabels.ROOM_TYPE_NAME_DUPLICATED, service.roomTypeInsert(data).getMessage());
	}
	...
	void UpdateBadColumnName() {
		...
		assertEquals(MsgLabels.BAD_DATA, service.roomTypeUpdate(data, filter).getMessage());
	}
	...
	void UpdateBlankName() {
		...
		assertEquals(MsgLabels.ROOM_TYPE_NAME_BLANK, service.roomTypeUpdate(data, filter).getMessage());
	}
	...
	void UpdateTestDuplicate() {
		...
		assertEquals(MsgLabels.ROOM_TYPE_NAME_DUPLICATED, service.roomTypeUpdate(data, filter).getMessage());
	}
	...
	void DeleteInexistentTest() {
		Map<String, Object> filter = new HashMap<String, Object>();
		filter.put(RoomTypeDao.ATTR_ID, 1);
		when(daoHelper.query(roomTypeDao, filter, Arrays.asList(RoomTypeDao.ATTR_ID)))
				.thenReturn(createResult(MsgLabels.NO_DATA_TO_DELETE));
	...
	void DeleteInUseTest() {
		...
		EntityResult result = service.roomTypeDelete(filter);
		assertEquals(EntityResult.OPERATION_WRONG, result.getCode());
		assertEquals(MsgLabels.ROOM_TYPE_IN_USE, result.getMessage());
	}

	public EntityResult createRoomTypeTemplate(int operationcode) {
		EntityResult template = new EntityResultMapImpl(Arrays.asList(RoomTypeDao.ATTR_ID, RoomTypeDao.ATTR_NAME,
				RoomTypeDao.ATTR_BED_NUMBER, RoomTypeDao.ATTR_BASE_PRICE));
